...
- (instancetype)initWithDataArr:(NSArray *)dataArr origin:(CGPoint)origin width:(CGFloat)width rowHeight:(CGFloat)rowHeight
{
    ...
    if (self) {
        ...
        [_tableView registerClass:[UITableViewCell class] forCellReuseIdentifier:@"CPFCustomMenu"];
...
{
    UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"CPFCustomMenu"];
...
{
    if([self.delegate respondsToSelector:@selector(CPFCustomMenu:didSelectRowAtIndexPath:)]){
        [self.delegate CPFCustomMenu:tableView didSelectRowAtIndexPath:indexPath];
    }
